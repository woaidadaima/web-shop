          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <!-- isChecked为字符串但是在关系运算符计算中会转化为数字 -->
                <dd
                  @click="
                    active(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)
                  "
                  changepirce="0"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  @change="inputValue"
                />
                <a @click="skuNumber++" class="plus">+</a>
                <a
...
export default {
  ...
  methods: {
    // 点击商品属性让商品属性高亮
    active(spuSaleAttrValue, spuSaleAttrValueList) {
      // 先干掉其他人，再留下自己
    ...
    async addShopcar() {
      // 派发请求调，传递商品ID和数量两个参数，传统promise写法
      // let result =  this.$store.dispatch("addOrUpdateShopCar", {
      //   id: this.skuInfo.id,
      //   skuNumber: this.skuNumber,
      // });
      // result.then((res) => {
      //     sessionStorage.setItem('store2',JSON.stringify(this.skuInfo))
      ...
      //派发请求，告诉服务器购买数量以及购买ID
      try {
        await this.$store.dispatch("addOrUpdateShopCar", {
          id: this.skuInfo.id,
          skuNumber: this.skuNumber,
        });
        //成功将skuInfo存到本地存储里，进行路由跳转
        sessionStorage.setItem("store2", JSON.stringify(this.skuInfo));
        this.$router.push({
          name: "addCartSuccess",
          query: { skuNumber: this.skuNumber },
        });
      } catch (error) {
        alert(error.message);
      }
    ...
    //获取用户输入的产品数量
    inputValue(event) {
      //任何字符串和数字相乘都是NAN，将用户输入的小数转化为整数
      this.skuNumber = parseInt(event.target.value * 1);
      //如果是NAN,NAN的布尔值为0
      if (!this.skuNumber) {
        // 则让输入数量重置为1
        this.skuNumber = 1;
